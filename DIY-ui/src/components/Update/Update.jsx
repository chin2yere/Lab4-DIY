...
export default function Update() {
  ...
  const [owner, setOwner] = useState(passedProps.owner)
  const [exterior, setExterior] = useState(passedProps.exterior)
  const [interior, setInterior] = useState(passedProps.interior)
  const [wheels, setWheels] = useState(passedProps.wheels)
  const [roof, setRoof] = useState(passedProps.roof)
  const [convertible, setConvertible] = useState(passedProps.convertible);
...
const handleUpdate = async (e) => {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:3000/car/${passedProps.id}`, {
      method: "PATCH",
      headers: {
    ...
    <div>
        <article>
          <div style={{textAlign: "center", color: "whitesmoke", marginTop: "40px", marginBottom:"40px"}}>
            ...
            <input defaultValue={passedProps.owner} style={{width:"300px", height:"50px", borderRadius:"16PX"}} type="text" 
            onChange={(e) => 
            {
        ...
        <button onClick={()=>{
          handleUpdate()
        }} className="button-create" style={{marginBottom:"50px", marginLeft:"190px",backgroundColor:"blueviolet"}}>Update</button>
    </div>
  );
}